...
Animation.prototype.setTool = function(Tool) {
  this.tool = new Tool(this.contexts);
};
...
Animation.prototype.nextStep = function() {
  ...
  this.currentStep = new Step(this.contexts);
...
Animation.prototype.processEvent = function(results) {
  if (results) {
    ...
    if (results.complete) {
      ...
      this.currentStep.replay("current");
      this.nextStep();
    } else {
      this.currentStep.add(results.data);
    }
...
Animation.prototype.flattenHistory = function() {
  ...
  if (history.length > this.settings.history.max) {
    ...
    this.frame.current.updateData(this.saveBuffer.canvas.toDataURL()); 
