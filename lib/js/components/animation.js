...
Animation.prototype.drawCurrentLayer = function() {
  ...
  if (this.frame.current.dataURL) {
    var img = new Image();
    img.src = this.frame.current.dataURL
...
Animation.prototype.setTool = function(Tool) {
  this.tool = new Tool(this.contexts.buffer, this.contexts.current);
}
...
Animation.prototype.nextStep = function() {
  if (this.currentStep) {
    ...
    this.frame.current.history.push(this.currentStep);
  }
  this.currentStep = new Step(this.contexts.buffer, this.contexts.current);
...
Animation.prototype.load = function(index) {
  ...
  for (var i=1; i <= index; i++) {
    this.frame.current.history[i - 1].replay();
  }
}
