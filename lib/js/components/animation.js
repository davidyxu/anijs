...
Animation.prototype.drawCurrentLayer = function(index) {
  ...
  for (var i=1; i <= index || this.frame.current.length; i++) {
...
Animation.prototype.setupCanvas = function(selector) {
  ...
  // todo refactor
  this.container.innerHTML = '<div id="layers"><canvas id="top-layers"></canvas><canvas id="buffer-layer"></canvas><canvas id="current-layer"></canvas><canvas id="bot-layers"></canvas></div><div id="onion-layer"><canvas class="onion" id="prev-onion"></canvas><canvas class="onion" id="next-onion"></canvas></div><canvas id="exporter" style="display: none;"></canvas>'
  ...
  this.contexts = {
    ...
    exporter: document.getElementById('exporter').getContext('2d')
  };
};
...
Animation.prototype.play = function(options) {
  if (!this.state.playing) {
    ...
    var delay = 1000 / (options.fps || this.fps) // ms delay
...
Animation.prototype.exportWebm = function(fps) {
  var video = new Whammy.Video(fps || this.fps);
  var img = new Image();

  var exporter = this.contexts.exporter;
  exporter.fillStyle = 'white'

  this.frames.forEach(function(frame) {
    exporter.fillRect(0,0, exporter.canvas.width, exporter.canvas.height);
    img.src = frame.dataURL;
    exporter.drawImage(img, 0, 0);
    video.add(exporter);
  }.bind(this));

  var data = video.compile();
  return (window.webkitURL || window.URL).createObjectURL(data);
}
